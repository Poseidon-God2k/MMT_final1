...
class Client:
    ...
    def chooseMovie(self, videoList):
        ...
        def go(event):
            self.fileName = chooseMovie.get(
                chooseMovie.curselection())
            self.sendRtspRequest(self.CHOOSE)

        chooseMovie = Listbox(self.master)
        chooseMovie.bind('<Double-1>', go)
        chooseMovie.grid(row=1, column=6)
    ...
    def writeFrame(self, data):
        ...
        # # accumulate the movie size
        self.movieSize += picSize
        # # print "movieSize: " + str(self.movieSize) + " byte"
        # # accumulate playing time
        self.timeConsume += time.time()-self.playingTime
